# ROOM ISP - agente seguro v4 para RouterOS 6.49+
...
:local roomStateSource (":global ROOM_ISP_ROUTER_ID \"" . $roomRouterId . "\";\r\n:global ROOM_ISP_AGENT_TOKEN \"" . $roomAgentToken . "\";\r\n:global ROOM_ISP_AGENT_URL \"" . $roomAgentUrl . "\";\r\n:global ROOM_ISP_PROTOCOL 4;");
/system script add name=ROOM_ISP_STATE comment="ROOM ISP v4 - credencial privada" policy=read source=$roomStateSource;

/system script add name=ROOM_ISP_AGENT comment="ROOM ISP v4 - control de acceso y cobros" policy=read,write,test source={
    :do {
        /system script run ROOM_ISP_STATE;
        :global ROOM_ISP_ROUTER_ID; :global ROOM_ISP_AGENT_TOKEN; :global ROOM_ISP_AGENT_URL;
        ...
        :local roomBody ("{\"protocol\":4,\"mode\":\"poll\",\"router_id\":\"" . $ROOM_ISP_ROUTER_ID . "\",\"health\":{\"cpu\":" . $roomCpu . ",\"free_memory\":" . $roomFree . ",\"uptime\":\"" . $roomUptime . "\"}}");
        :local roomHeaders ("Content-Type:application/json,Authorization:Bearer " . $ROOM_ISP_AGENT_TOKEN);
        :local roomFetch [/tool fetch url=$ROOM_ISP_AGENT_URL http-method=post http-header-field=$roomHeaders http-data=$roomBody output=user as-value check-certificate=yes-without-crl];
        ...
        :if ([:pick $roomData 0 5] != "NONE|") do={
            ...
            :local resultBody ("{\"protocol\":4,\"mode\":\"result\",\"router_id\":\"".$ROOM_ISP_ROUTER_ID."\",\"command_id\":\"".$commandId."\",\"success\":" . $success . ",\"result\":\"".$result."\"}");
            :do { /tool fetch url=$ROOM_ISP_AGENT_URL http-method=post http-header-field=$roomHeaders http-data=$resultBody output=user as-value check-certificate=yes-without-crl; } on-error={ :log warning "ROOM ISP: ACK pendiente"; };
        };
    } on-error={ :log warning "ROOM ISP: backend no disponible; la red sigue operando"; };
};

/system script add name=ROOM_ISP_INVENTORY comment="ROOM ISP v4 - inventario por lotes sin passwords" policy=read,test source={
    :do {
        /system script run ROOM_ISP_STATE;
        :global ROOM_ISP_ROUTER_ID;
        :global ROOM_ISP_AGENT_TOKEN;
        :global ROOM_ISP_AGENT_URL;
        ...
        :local roomSend do={
            ...
            :local body ("{\"protocol\":4,\"mode\":\"inventory\",\"router_id\":\"" . $ROOM_ISP_ROUTER_ID . "\",\"sequence\":" . $sequence . ",\"final\":" . $final . ",\"items\":[" . $items . "]}");
            :local headers ("Content-Type:application/json,Authorization:Bearer " . $ROOM_ISP_AGENT_TOKEN);
            :local f [/tool fetch url=$ROOM_ISP_AGENT_URL http-method=post http-header-field=$headers http-data=$body output=user as-value check-certificate=yes-without-crl];
...
/system scheduler add name=ROOM_ISP_AGENT_SCHED interval=$roomPollInterval start-time=startup on-event="/system script run ROOM_ISP_AGENT" policy=read,write,test comment="ROOM ISP v4 - poll";
/system scheduler add name=ROOM_ISP_INVENTORY_SCHED interval=1d start-time=00:17:00 on-event="/system script run ROOM_ISP_INVENTORY" policy=read,test comment="ROOM ISP v4 - inventario diario";
